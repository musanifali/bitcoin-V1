import Header from '@/components/layout/Header'
import Footer from '@/components/layout/Footer'
import { Button } from '@/components/ui/button'
import { Code, Book, Github, Zap } from 'lucide-react'

export default function Development() {
  return (
    <main className="min-h-screen bg-background">
      <Header />
      
      <section className="pt-24 pb-16 bg-gradient-to-br from-bitcoin-500/10 via-background to-orange-500/10">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-6">
              Build on 
              <span className="block bg-gradient-to-r from-bitcoin-500 via-orange-500 to-bitcoin-600 bg-clip-text text-transparent">
                Bitcoin V1
              </span>
            </h1>
            <p className="text-xl text-muted-foreground mb-8 leading-relaxed">
              Developer resources and tools for building Bitcoin V1 applications.
            </p>
            <Button size="lg" className="bg-bitcoin-500 hover:bg-bitcoin-600">
              Start Building
            </Button>
          </div>
        </div>
      </section>

      <section className="py-16">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              <div className="text-center">
                <Code className="h-16 w-16 mx-auto text-bitcoin-500 mb-4" />
                <h3 className="text-xl font-bold mb-4">APIs & SDKs</h3>
                <p className="text-muted-foreground">Comprehensive development tools</p>
              </div>
              <div className="text-center">
                <Book className="h-16 w-16 mx-auto text-bitcoin-500 mb-4" />
                <h3 className="text-xl font-bold mb-4">Documentation</h3>
                <p className="text-muted-foreground">Complete developer guides</p>
              </div>
              <div className="text-center">
                <Github className="h-16 w-16 mx-auto text-bitcoin-500 mb-4" />
                <h3 className="text-xl font-bold mb-4">Open Source</h3>
                <p className="text-muted-foreground">Contribute to the codebase</p>
              </div>
              <div className="text-center">
                <Zap className="h-16 w-16 mx-auto text-bitcoin-500 mb-4" />
                <h3 className="text-xl font-bold mb-4">Lightning Network</h3>
                <p className="text-muted-foreground">Build instant payment apps</p>
              </div>
            </div>
          </div>
        </div>
      </section>
      
      <Footer />
    </main>
  )
}
